...
class AIContext:
    ...
    def make_key(self, action, command, logs=""):
    ...
    async def ask_ai(self, system_prompt, user_content, max_tokens=1000, cache_key=None):
        if not self.api_key:
    ...
    async def analyze_results(self, tool, logs, command=""):
        ...
        h = self.make_key("analyze", command, logs)
        ...
        return await self.ask_ai(system, user, max_tokens=max_t, cache_key=h)
    ...
    async def explain_command(self, command):
        ...
        return await self.ask_ai(system, user, max_tokens=350, cache_key=h)

    #BOTÃO: Dicas e Passos 
    async def get_tool_tips(self, tool, phase, command="", logs=""):
        logs = normalize_logs(logs, tool)
        h = self.make_key("tips", f"{tool}:{command}", logs)
        ...
        return await self.ask_ai(system, user, max_tokens=600, cache_key=h)

    #BOTÃO: Adicionar ao Relatório 
    async def generate_formal_report(self, tool, logs, command=""):
        logs = normalize_logs(logs, tool)
        h = self.make_key("report", f"{tool}:{command}", logs)
        system = (
            f"{TEACHER_JAMERSON}\n"
    ...
    async def generate_executive_summary(self, findings):
        ...
        return await self.ask_ai(system, f"Achados:\n{findings}", max_tokens=1000, cache_key=h)
